...
# ── Data Generation ─────────────────────────────────────────────
def gen_prices(n=200, seed=42):
    """Bull (30%) -> Plateau (20%) -> Crash (10%) -> Recovery (25%) -> Chop (15%)"""
    np.random.seed(seed)
    p = []
    for frac, start_mul, end_mul, vol in [
        ...
        arr = np.linspace(s, e, ln) + np.random.randn(ln) * vol
        p.extend(arr.tolist())
    return np.array(p[:n])

def gen_shock_prices(n=200, seed=42):
    """Same as gen_prices but with a sharp crash spike at step 60."""
    p = gen_prices(n, seed)
    # Inject sudden -15% crash
    crash_idx = int(len(p) * 0.40)
    spike = np.zeros(min(8, len(p) - crash_idx))
    spike[0] = -p[crash_idx] * 0.15
    p[crash_idx:crash_idx + len(spike)] += spike[:len(p) - crash_idx]
    # Sharp V-shape recovery after
    rec_start = p[crash_idx + 4] if crash_idx + 4 < len(p) else p[-1]
    for j in range(crash_idx + 5, min(crash_idx + 20, len(p))):
        p[j] = p[j - 1] + (p[crash_idx - 5] - rec_start) / 15
        p[j] += np.random.randn() * 1.5
    return p
...
def main():
    ...
    prices = gen_shock_prices(200, 42)
    prev_action = "hold"
    max_consec = 0
    safety_events = 0
    mtm = 10000.0
